...
async def set_cookie(request: Request, data: Cookie):
    """
...
async def edit_cookie(data: Cookie):
    """
    Edits a cookie in the response.
    """
    response = JSONResponse(status_code=200, content={"message": "Edit Cookie", "data": data})
    response.set_cookie(
        key=data.name,            # Name
        value=data.value,         # Payload
        httponly=True,            # kein JS-Zugriff
...
async def delete_cookie(data: Cookie):
    """
